/*
  ...
  3.  doubly linked list 와 화일 처리 를 활용한 객실 관리 프로그램 작성
      첨부화일(객실예약.txt)을 참고 하여 아래의 기능이 동작 되도록 구현 하시오
      ---- requirement ----
      3.1 현재의 구조는 reservation.txt화일의 내용을 단순 array에 저장을 하도록 되어 있으나 doubly linked list 메모리 구조로 저장 하도록 한다.
      3.2 input(), delete(), modify() one_person_display() all_display() exit_program() 각각 동작 되도록 구현 한다. 
      3.3 t_NODE NODE[10]가 정적 순차 배열 로 되어 있는것을 동적메모리 double linked list로 구현 하고, 
          전역변수로 선언되어 있는 double linked list 를 main함수 내에 위치 하는 지역변수로 변경하여 구조체 포인터로 동작하도록.
      3.4 함수 호출을 switch~case문으로 되어 있는 것을 함수 포인터 배열과 함수 포인터로 호출 되도록 프로그램을 변경한다.
